...
public class MentoringCsvDataAdapter implements MentoringCsvDataOutPort {
    ...
    // 멘토링 세션, 멘토링 uuid 반환
    public void sendMentoringDto(){
        List<String> mentoringUuid = mentoringJpaRepository.findAllMentoringUuids();

        List<String> sessionUuid = mentoringUuid.stream()
                .flatMap(uuid -> mentoringJpaRepository.findSessionUuidsByMentoringUuid(uuid).stream())
                .toList();

        sendMessageUseCase.sendMentoringData("mentoring-data",MentoringDataDto.of(mentoringUuid,sessionUuid));
    }
